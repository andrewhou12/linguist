...
import { Box, Heading, Text, Flex, TextField, Badge, Table, Select } from '@radix-ui/themes'
import { Search } from 'lucide-react'
import { useWordbank } from '@/hooks/use-wordbank'
import { Skeleton } from '@/components/skeleton'
import { MasteryState } from '@linguist/shared/types'
import type { WordBankEntry } from '@linguist/shared/types'
...
export default function KnowledgePage() {
  ...
  const handleSearch = (value: string) => {
    ...
    if (value.trim()) { search(value.trim()) } else { reload() }
  }

  const handleMasteryFilter = (value: string) => {
    setMasteryFilter(value)
    setSearchQuery('')
    if (value === 'all') { setFilters(undefined) } else { setFilters({ masteryState: value as MasteryState }) }
  }
