...
import type { AssessmentItem, OnboardingResult, SelfReportedLevel } from '@linguist/shared/types'
...
export default function OnboardingPage() {
  ...
  const handleComplete = useCallback(async () => {
    ...
    try {
      ...
      await api.onboardingComplete(result)
      router.push('/dashboard')
    } finally {
      setSubmitting(false)
    }
  }, [nativeLanguage, selfReportedLevel, dailyNewItemLimit, knownItems, router])
      ...
      <Flex direction="column" align="center" gap="5" p="4">
        ...
        <Flex direction="column" align="center" gap="2">
          ...
          <Text size="3" color="gray" align="center" style={{ maxWidth: 400 }}>
            Let&apos;s set up your personalized learning experience. We&apos;ll ask a few questions to understand your current level and tailor your study plan.
          </Text>
        </Flex>
        <Text size="2" color="gray" align="center">This takes about 2 minutes.</Text>
...
function LanguageStep({ nativeLanguage, onNativeLanguageChange, onNext, onBack }: {
  nativeLanguage: string; onNativeLanguageChange: (lang: string) => void; onNext: () => void; onBack: () => void
}) {
...
function LevelStep({ onSelect, onBack }: { onSelect: (level: SelfReportedLevel) => void; onBack: () => void }) {
    ...
    <Card size="4" style={{ width: '100%' }}>
      <Flex direction="column" gap="5" p="4">
        <Flex direction="column" gap="2">
          ...
          <Text size="2" color="gray">Don&apos;t worry about being exact — we&apos;ll fine-tune this with a quick vocabulary check next.</Text>
        </Flex>
        <Flex direction="column" gap="2">
          {levels.map((level) => (
            <div
              ...
              style={{
                cursor: 'pointer', padding: '12px 16px', borderRadius: 'var(--radius-2)',
...
function AssessmentStep({ items, knownItems, loading, onToggle, onNext, onBack }: {
  items: AssessmentItem[]; knownItems: Set<number>; loading: boolean
  onToggle: (index: number) => void; onNext: () => void; onBack: () => void
}) {
              ...
              <Flex direction="column" gap="3">
                ...
                <Flex gap="2" wrap="wrap">
                  {vocabItems.map((item) => {
                    ...
                    return (
                      <Button key={item.index} variant={isKnown ? 'solid' : 'outline'} size="2" onClick={() => onToggle(item.index)} style={{ cursor: 'pointer', transition: 'all 0.15s ease' }}>
...
function PreferencesStep({ dailyNewItemLimit, onDailyLimitChange, onNext, onBack }: {
  dailyNewItemLimit: number; onDailyLimitChange: (limit: number) => void; onNext: () => void; onBack: () => void
}) {
  return (
    <Card size="4" style={{ width: '100%' }}>
      <Flex direction="column" gap="5" p="4">
        ...
        <Card variant="surface">
          <Flex direction="column" gap="2" p="2">
            ...
            <Text size="1" color="gray">
              Each day, up to {dailyNewItemLimit} new vocabulary and grammar items will be introduced alongside your review queue. A higher number means faster progress but requires more daily study time.
            </Text>
          </Flex>
        </Card>
        <Flex gap="3" justify="between">
...
function CompleteStep({ knownCount, totalCount, level, submitting, onComplete, onBack }: {
  knownCount: number; totalCount: number; level: SelfReportedLevel; submitting: boolean
    ...
    <Card size="4" style={{ width: '100%' }}>
      <Flex direction="column" align="center" gap="5" p="4">
        ...
        <Flex gap="3" style={{ width: '100%' }} direction="column">
          <Button size="3" onClick={onComplete} disabled={submitting} style={{ width: '100%', cursor: submitting ? 'wait' : 'pointer' }}>
