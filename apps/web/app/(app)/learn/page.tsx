...
export default function LearnPage() {
  ...
  const fetchPlanningData = useCallback(async () => {
    ...
    try {
      const [bubbleData, recs] = await Promise.all([
        api.curriculumGetBubble(),
        api.curriculumGetRecommendations(),
      ])
  ...
  const handleSkip = useCallback(async (id: number) => {
    try {
      await api.curriculumSkipItem(id)
    } catch (err) {
  ...
  const handleRefresh = useCallback(async () => {
    setIsLoading(true)
    try {
      const recs = await api.curriculumRegenerate()
      setRecommendations(recs)
    } catch (err) {
  ...
  const handleStart = useCallback(async () => {
    ...
    try {
      ...
      for (const rec of toIntroduce) {
        try {
          await api.curriculumIntroduceItem(rec.id!)
        } catch {
