...
import type { ConversationMessage, ExpandedSessionPlan, PragmaticState, FsrsState } from '@lingle/shared/types'
import type { Prisma } from '@prisma/client'
import { buildAnalysisPrompt, parseAnalysis } from '@lingle/core/conversation/analyzer'
import { buildPragmaticAnalysisPrompt, parsePragmaticAnalysis, updatePragmaticState } from '@lingle/core/pragmatics/analyzer'
import { createInitialFsrsState, scheduleReview } from '@lingle/core/fsrs/scheduler'
...
export const POST = withAuth(async (request, { userId }) => {
  ...
  // Fetch learner's known vocabulary so the analysis can identify truly new items
  const knownItems = await prisma.lexicalItem.findMany({
    where: { userId },
    select: { surfaceForm: true },
  })
  const knownSurfaceForms = knownItems.map((i) => i.surfaceForm)

  // Post-session analysis
  const analysisPrompt = buildAnalysisPrompt(transcript, plan, knownSurfaceForms)
